...
/**
 * @author stevec
 *         This class extends the JApplet class to create an applet that
 *         estimates the fractal dimension
 *         of a point set. The point set is provided in a .csv file or extracted
 *         from a binary image.
 *         The program will attempt to extract a point set from a non-binary
 *         image, but results may
 *         vary.
 */
public class FracDim extends JFrame implements ActionListener {
	private static final long serialVersionUID = 7720616729741130658L;
	private static final int PREFERRED_IMAGE_WIDTH = 800;
	private static final int PREFERRED_IMAGE_HEIGHT = 800;
	...
	/**
	 * @author stevec
	 *         This class extends JPanel to create a panel which, when repainted,
	 *         draws the
	 *         fractal image.
	 */
	...
	/**
	 * The init method initializes variables, tries to set the image and display
	 * panel sizes,
	...
	/*
	 * This method comes into play when the user presses the open button. It queries
	 * the user for
	 * a file. It then checks the file extension to see whether it appears to be a
	 * csv file.
	 * If so, it extracts the points from the file. Otherwise, it opens the file as
	 * an image and
	 * extracts points from the image. It then computes and displays the fractal
	 * dimension of the
	 * point set.
	 */
			...
			if (fCap.endsWith("CSV")) {
				try {
					BufferedReader reader = new BufferedReader(
							new FileReader(fd.getDirectory() + File.separator + fd.getFile()));
					int i = 0;
					String text = null;
					pSet = new PointSet();
